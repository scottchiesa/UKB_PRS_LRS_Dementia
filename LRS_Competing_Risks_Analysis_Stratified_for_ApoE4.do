...
***************************************************************************
***************LRS Analyses Stratified for ApoE4 Carriage******************
***************************************************************************
...
***************************************************************************
*******************INDIVIDUALS WITHOUT APOE4 CARRIAGE**********************
***************************************************************************

drop if e4_carrier == 1
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		keep(z_LRS_imputed age sex edu socio_strat drug_any_bpdrug_ni_bl drug_statin_ni_bl dm_anydmrx_ni_sr_bl e4_carrier) trans(1)
		...
		quietly {
			...
			log using "LRS_allcause_noe4.csv", replace text name(table)
...
import delimited "LRS_allcause_noe4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_allcause_noe4_results, replace
		restore
	}
...
**LRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		stset allcause_date, failure(allcause_new=1,2) id(n_eid) scale(365.25)
		stcrprep, events(allcause_new) ///
		keep(LRS_tertile_imputed age sex edu socio_strat drug_any_bpdrug_ni_bl drug_statin_ni_bl dm_anydmrx_ni_sr_bl e4_carrier) trans(1)
		gen allcause_event = failcode == allcause_new
		stset tstop [pw=weight_c], failure(allcause_event==1) enter(tstart)
		stcox i.LRS_tertile_imputed age i.edu i.socio_strat i.drug_any_bpdrug_ni_bl i.drug_statin_ni_bl i.dm_anydmrx_ni_sr_bl, strata(sex)
		...
		display as text "SHR for LRS_tertile_2 = " `shr2_`i''
		display as text "SE for LRS_tertile_2 = " `se2_`i''
		display as text "LCI for LRS_tertile_2 = " `ll2_`i''
		display as text "UCI for LRS_tertile_2 = " `ul2_`i''
		display as text "P for LRS_tertile_2 = " `pval2_`i''
		display as text "SHR for LRS_tertile_3 = " `shr3_`i''
		display as text "SE for LRS_tertile_3 = " `se3_`i''
		display as text "LCI for LRS_tertile_3 = " `ll3_`i''
		display as text "UCI for LRS_tertile_3 = " `ul3_`i''
		display as text "P for LRS_tertile_3 = " `pval3_`i''
		display as text "Number of observations = " as result e(N)
		
		quietly {
			capture log close table
			log using "LRS_allcause_tertiles_noe4.csv", replace text name(table)
...
import delimited "LRS_allcause_tertiles_noe4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_allcause_tertiles_noe4_results, replace
		restore
				}
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		keep(z_LRS age sex edu socio_strat drug_any_bpdrug_ni_bl drug_statin_ni_bl dm_anydmrx_ni_sr_bl e4_carrier) trans(1)
		gen alzheimer_event = failcode == alzheimer_new
		stset tstop [pw=weight_c], failure(alzheimer_event==1) enter(tstart)
		stcox z_LRS age i.sex i.edu i.socio_strat i.drug_any_bpdrug_ni_bl i.drug_statin_ni_bl i.dm_anydmrx_ni_sr_bl, strata(sex)
		...
		local ul`i' = mat`i'[6,1]
		local pval`i' = mat`i'[4,1]
		display as text "SHR for LRS = " `shr`i''
		display as text "SE for LRS = " `se`i''
		display as text "LCI for LRS = " `ll`i''
		display as text "UCI for LRS = " `ul`i''
		display as text "P for LRS = " `pval`i''
		display as text "Number of observations = " as result e(N)
		
		quietly {
			capture log close table
			log using "LRS_alzheimers_noe4.csv", replace text name(table)
...
import delimited "LRS_alzheimers_noe4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_alzheimers_noe4_results, replace
		restore
	}
...
**LRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		gen alzheimer_event = failcode == alzheimer_new
		stset tstop [pw=weight_c], failure(alzheimer_event==1) enter(tstart)
		stcox i.LRS_tertile_imputed age i.sex i.edu i.socio_strat i.drug_any_bpdrug_ni_bl i.drug_statin_ni_bl i.dm_anydmrx_ni_sr_bl, strata(sex)
		matrix mat`i' = r(table)
		matrix list mat`i'
		...
		quietly {
			capture log close table
			log using "LRS_alzheimers_tertiles_noe4.csv", replace text name(table)
...
import delimited "LRS_alzheimers_tertiles_noe4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_alzheimers_tertiles_noe4_results, replace
		restore
				}
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_vascular_noe4.csv", replace text name(table)
...
import delimited "LRS_vascular_noe4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_vascular_noe4_results, replace
		restore
	}
...
**PRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_vascular_tertiles_noe4.csv", replace text name(table)
...
import delimited "LRS_vascular_tertiles_noe4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_vascular_tertiles_noe4_results, replace
		restore
		}
...
***************************************************************************
*********************INDIVIDUALS WITH APOE4 CARRIAGE***********************
***************************************************************************

use "Dataset_ready_for_imputation_analyses.dta", clear

drop if e4_carrier == 0
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_allcause_e4.csv", replace text name(table)
...
import delimited "LRS_allcause_e4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_allcause_e4_results, replace
		restore
	}
...
**LRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_allcause_tertiles_e4.csv", replace text name(table)
...
import delimited "LRS_allcause_tertiles_e4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_allcause_tertiles_e4_results, replace
		restore
				}
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_alzheimers_e4.csv", replace text name(table)
...
import delimited "LRS_alzheimers_e4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_alzheimers_e4_results, replace
		restore
	}
...
**LRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_alzheimers_tertiles_e4.csv", replace text name(table)
...
import delimited "LRS_alzheimers_tertiles_e4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_alzheimers_tertiles_e4_results, replace
		restore
				}
...
**LRS**	
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_vascular_e4.csv", replace text name(table)
...
import delimited "LRS_vascular_e4.csv", delimiter(space) clear 
egen LRS_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_vascular_e4_results, replace
		restore
	}
...
**PRS Tertiles**
	...
	forvalues i = 1/10	{
		...
		quietly {
			capture log close table
			log using "LRS_vascular_tertiles_e4.csv", replace text name(table)
...
import delimited "LRS_vascular_tertiles_e4.csv", delimiter(space) clear 
egen LRS_tertiles_Results = rowmean(v2 v3 v4 v5 v6 v7 v8 v9 v10 v11) 
save LRS_vascular_tertiles_e4_results, replace
		restore
		}
